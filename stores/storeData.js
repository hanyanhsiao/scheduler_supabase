...
export const useStoreData = defineStore('storeData', {
  ...
  actions: {
    ...
    // 1 獲取所有課程
    async getClassData() {
      // const response = await fetch('https://0fc9-123-51-220-96.ngrok-free.app/course')
    ...
    async getSubject() {
      if (this.subjectOptions.length === 0) {
        // const subjectResponse = await fetch('https://0fc9-123-51-220-96.ngrok-free.app/subject')
    ...
    async modifyClass(input) {
      // -----------API-------------
      const res = await fetch(`https://0fc9-123-51-220-96.ngrok-free.app/course/${input.id}`, {
        method: 'PUT',
    ...
    async addClass(input) {
      ...
      await fetch(`https://0fc9-123-51-220-96.ngrok-free.app/course`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    async deleteClass(input, index) {
      ...
      if (yes) {
        // -----------API-------------
        await fetch(`https://0fc9-123-51-220-96.ngrok-free.app/course/${input.id}`, {
          method: 'DELETE'
        }).then((res) => {
          return res.json()
        })
